...
export function useDriverStandingsEvolution(year) {
  ...
  const { data: standingsRows = [], isLoading: isLoadingStandings } = useQuery({
    queryKey: ["championship_drivers_evolution", { session_keys: sessionKeys }],
    queryFn: () => openF1Api.championshipDrivers({ session_key: sessionKeys }),
  ...
  }, [completedRounds, driverList, standingsBySession, isLoading]);
}
