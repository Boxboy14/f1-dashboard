...
3. **Given** any session detail page, **When** it loads, **Then** the session name, type (e.g., Race, Qualifying, Sprint), and date are shown as a page heading.
...
As an F1 fan, I want to see the complete pit stop history for a session — which driver stopped, on which lap, and how long the stop took — so I can analyse race strategy.
...
**Independent Test**: Navigate to a completed race session. Confirm a pit stops table is shown listing each driver's stops with lap number and duration in seconds. For non-race sessions (e.g., qualifying) with no pit data, confirm the table is absent or shows "No pit stops recorded."

**Acceptance Scenarios**:

1. **Given** a race session with pit stop data, **When** the user views the session detail, **Then** a pit stops table shows driver name, team, lap number, and pit duration.
2. **Given** a session with no pit stop data (qualifying, practice, or a race with no recorded stops), **When** the user views the session detail, **Then** the pit stops section is absent or displays "No pit stops recorded."
3. **Given** a driver with multiple pit stops in one race, **When** the user views the pit stops table, **Then** all stops for that driver are listed as separate rows.
...
- **FR-001**: The page MUST display the session name, session type, and scheduled date/time in a header area.
...
- **FR-005**: The page MUST show a pit stops section when pit stop data is available, containing driver name, team, lap number, and pit duration.
...
- **Session**: Identified by `session_key`; carries `session_name`, `session_type`, `date_start`, `meeting_key` for back-navigation.
...
- The OpenF1 `/session_result` endpoint is the authoritative source for classification data; `/pit` is the source for pit stop data.
