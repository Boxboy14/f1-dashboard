...
- **FR-016**: The page MUST retain the current selection (event, session, drivers, lap) for the duration of the browser session — surviving a page refresh and navigation away to other tabs and back — and MUST restore it on return. The retained selection MUST be cleared when the session ends (the tab/app is closed) and MUST reset when the navbar season changes.
...
- **Selection persistence** uses session-scoped browser storage (cleared on tab close); it is distinct from the app's existing query-cache persistence (which caches API responses across sessions).
- **Presentation**: selectors are laid out as a form (labelled fields, two columns); each driver summary card shows the driver's photo, name, and team alongside the lap figures and occupies half the width for a single driver; the telemetry charts are spaced apart with the metric name shown as a vertical axis label.
